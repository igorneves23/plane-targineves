  ...
  return (
    <div className="space-y-2">
      ...
      <div className="relative">
        ...
        {open && (
          <div className="absolute top-full left-0 mt-1 w-44 bg-bg2 border border-bdr/10 rounded-lg shadow-xl z-20 py-1 max-h-48 overflow-y-auto">
            {labels.map((label) => (
              <button
                key={label.id}
                onClick={() => toggle(label)}
                className="w-full flex items-center gap-2 px-3 py-2 hover:bg-bdr/5 transition-colors"
              >
                <span className="w-3 h-3 rounded-full shrink-0" style={{ backgroundColor: label.color }} />
                <span className="text-xs text-tx2 flex-1 text-left">{label.name}</span>
